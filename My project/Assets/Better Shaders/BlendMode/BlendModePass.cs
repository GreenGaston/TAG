...
public class BlendModePass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        var materials = BlendModeMaterial.Instance;
        if (materials == null)
        {
        ...
        //---Custom effect here---
        var customEffect = stack.GetComponent<BlendModeVariables>();
        // Only process if the effect is active
        if (customEffect.IsActive())
        {
            var Mat= materials.customEffect;
            //write effects here
            Mat.SetInt("_BlendType", (int)customEffect.BlendType.value);
            Mat.SetVector("_BlendColor", customEffect.BlendColor.value);
            Mat.SetTexture("_BlendTexture", customEffect.BlendTexture.value);
            Mat.SetFloat("_Strength", customEffect.strength.value);

            Blit(cmd, source, destinationA, Mat, customEffect.BlendMode.value);
