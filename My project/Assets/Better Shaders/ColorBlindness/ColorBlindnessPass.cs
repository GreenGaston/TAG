...
public class ColorBlindnessPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        //---Custom effect here---
        var customEffect = stack.GetComponent<ColorBlindnessVariables>();
        // Only process if the effect is active
        if (customEffect.IsActive())
        {
            //write effects here
            var mat= materials.customEffect;
            mat.SetFloat("_Severity", customEffect.severity.value);
            mat.SetInt("_Difference", customEffect.difference.value?1:0);

            Blit(cmd,source,destinationA,mat,(int)customEffect.BlindType.value);
